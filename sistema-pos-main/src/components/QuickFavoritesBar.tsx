...
export const QuickFavoritesBar: React.FC<QuickFavoritesBarProps> = ({
  ...
  return (
    <div className="bg-[#FFF9F0]/85 backdrop-blur-md rounded-none p-3.5 border border-white/70 border-t-2 border-t-[#214C6A] shadow-[0_4px_20px_rgba(33,76,106,0.06)] space-y-3 text-[#222E3A] transition-all">
      {/* Header with Title and Customization Actions */}
      <div className="flex items-center justify-between px-1 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-none bg-white/70 backdrop-blur-xs border border-[#214C6A]/20 flex items-center justify-center text-[#214C6A] shadow-xs">
            <Star className="w-3.5 h-3.5 fill-[#EB9D52] text-[#BC6343]" />
          </div>
          <h3 className="text-xs font-black uppercase tracking-wider text-[#214C6A] font-['Outfit',sans-serif]">
            Botones Rápidos • Los Más Vendidos
          </h3>
          <span className="text-[10px] text-[#214C6A] bg-white/70 backdrop-blur-xs px-2 py-0.5 rounded-none border border-[#214C6A]/20 hidden sm:inline-block font-bold shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          ...
          {favoriteProducts.length > 0 && (
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-none text-[11px] font-bold transition-all cursor-pointer border backdrop-blur-xs shadow-2xs ${
                isEditMode
                  ? 'bg-[#EB9D52] text-[#222E3A] border-[#BC6343] shadow-xs'
                  : 'bg-white/60 hover:bg-white text-[#56291D] border-white/80 hover:border-[#214C6A]/30'
              }`}
            >
              {isEditMode ? (
                <>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 rounded-none bg-[#214C6A]/95 hover:bg-[#1a3d55] text-[#FFF9F0] text-[11px] font-bold shadow-xs transition-all cursor-pointer border border-[#214C6A] active:scale-95 backdrop-blur-xs"
          >
      ...
      {/* Quick Cards Grid with subtle glass effect */}
      {favoriteProducts.length === 0 ? (
        <div className="text-center py-6 px-4 bg-white/40 backdrop-blur-sm rounded-none border border-dashed border-[#214C6A]/30 flex flex-col items-center justify-center gap-2 shadow-inner">
          ...
          {favoriteProducts.map((product) => (
            <div key={product.id} className="relative group">
              <button
                ...
                className={`w-full p-2 rounded-none bg-white/75 hover:bg-white/95 backdrop-blur-md border transition-all flex flex-col items-center text-center cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.03)] hover:shadow-md active:scale-[0.98] text-left relative overflow-hidden ${
                  isEditMode
                    ? 'border-[#EB9D52] ring-2 ring-[#EB9D52]/40'
              ...
              >
                ...
                {/* Thumbnail */}
                <div className="w-11 h-11 rounded-none overflow-hidden border border-[#214C6A]/15 mb-1.5 shadow-2xs bg-white/90 shrink-0">
                  <img
                ...
                <div className="mt-1 w-full bg-[#BC6343]/10 backdrop-blur-xs border border-[#BC6343]/20 py-0.5 px-1 rounded-none text-center shadow-2xs">
